...
export default function CustomerDetailCard() {
  ...
  useEffect(() => {
    ...
    // L'interface Transaction ne contient pas d'id client: on filtre par l'id de transaction
    // (comportement de démo).
    const clientTx = TransactionData.filter(tx => tx.id === customerId);
    setTransactions(clientTx);

    if (!c) {
      // Redirection sécurisée si client introuvable
  ...
  if (!customer) return null; // ⚡ sécurise le rendu
  ...
  const columns: Column<Transaction>[] = [
    ...
    },
    {
      ...
      accessor: "status",
      render: (status) => (
        <Badge size="sm" color={status === "Completed" ? "success" : "warning"}>
          {String(status)}
        </Badge>
      ),
    },
  ...
  return (
    <div className="space-y-6">
      ...
      <ComponentCard title={`Fiche client: ${customer.name}`}>
        ...
        {error && <AlertMessage type="error" message={error} isOpen={true} onClose={() => {}} />}
